...
def objectives():
    ...
    if request.method == "POST":
        ...
        if pData.get("type") == "add":
            ...
            insertLearningObjective(topic_id, description, bloom, applicant, apprentice, journeyman, senior, chief,
                                    coordinator, tags)
        ...
        if pData.get("type") == "edit":
            ...
            db.engine.execute(query, topic_id=topic_id, description=description, bloom=bloom, applicant=applicant,
                              apprentice=apprentice, journeyman=journeyman, senior=senior, chief=chief,
                              coordinator=coordinator, tags=tags, objId=objId)
        elif pData.get("type") == "delete":
...
def filter_data():
    ...
    test_result = db.engine.execute(complete_query, topic_id=topic_id, subject_id=subject_id, start_date=start_date,
                                    end_date=end_date, search_name=search_name)
...
def tester_list(test_id):
    ...
    return render_template("tester_list.html", tester_data=tester_list_data, testId=test_id,
                           all_tester_data=all_tester_data)
...
def update_date():
    ...
    # Update attempt date
    update_query = text(
        "UPDATE test_scores SET attempt_date = :new_date WHERE test_id = :test_id and score_id = :score_id and tester_id = :tester_id")
...
def update_status():
    ...
    # Update pass status
    update_query = text(
        "UPDATE test_scores SET pass_status = :new_status WHERE test_id = :test_id and score_id = :score_id and tester_id = :tester_id")
...
def handle_get_questions():
    ...
    try:
        ...
        if test_type == 'manual':
            ...
            logging.debug(
                f"Filter parameters: blooms_taxonomy: {blooms_taxonomy}, subjects: {subjects}, topics: {topics}, question_types: {question_types}, question_difficulties: {question_difficulties}, training_level_conditions: {training_level_conditions}, keywords: {keywords}, question_max_points: {question_max_points}")
            response, status_code = get_questions(test_type, blooms_taxonomy, subjects, topics, question_types,
                                                  question_difficulties, training_level_conditions, keywords,
                                                  question_max_points)
        else:
            test_max_points = int(data.get('test_max_points', 0))
            logging.debug(
                f"Filter parameters: blooms_taxonomy: {blooms_taxonomy}, subjects: {subjects}, topics: {topics}, question_types: {question_types}, question_difficulties: {question_difficulties}")
...
def validate_questions_pool(questions_pool, num_questions, test_max_points):
    ...
    logging.info(
        "Check 1: Performing check to see if the number of questions available in the pool can meet the requirements of the requested number of questions and maximum point total.")
    ...
    logging.info(
        "Check 2: Performing check to see if the questions available in the questions pool can meet the user defined criteria of num_questions and test_max_points")
...
def handle_test_creation():
    ...
    try:
        ...
        response, status_code = create_test(is_active, created_by, creation_date, test_name, test_description,
                                            total_score, question_order)
...
def handle_test_creation_for_modify():
    ...
    try:
        ...
        # Create the test using extracted data
        test_message = create_test_for_modify(is_active, modified_by, last_modified_date, test_name, test_description,
                                              total_score, question_order, test_id)
    ...
    if request.method == "POST":
        ...
        if row:
            ...
            msg = Message('Training Test Program Email Verification.', sender=app.config['MAIL_USERNAME'],
                          recipients=[user['email']])
            msg.body = ('Dear ' + user['name'] +
                        '\n\nWe received a request to verify your new account ' + user[
                            'username'] + '. Please disregard this email if you did not register an account.' +
...
def verify_token():
    ...
    error_message = {'category': 'error',
                     'message': 'Invalid or expired verification link.<br>Please make sure you are using the correct link provided in your email.'}
