...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  const modifyTestButton = document.getElementById('modify_test_button')
  ...
  function initializeApp() {
    ...
    modifyTestButton.addEventListener('click', function (event) {
      // Prevent the default form submission behavior
      event.preventDefault();

      // Call the function to handle test modification
      handleTestModification();
    });
    openSidebarAndPopulateQuestions(testId)
  }

  // ---------- Function for populating side panel with selected test's data ----------
  ...
  // ---------- Function for modifying tests  ----------
  // Function to handle submit button click
  function handleTestModification() {
    // Log the initiation of the test modification process
    console.log('Starting test modification process.')
    ...
    const testModificationData = collectInputs();
    
    // Log the initiation of test creation attempt
    console.log('Attempting test modification.');
    ...
    fetch('/handle_test_creation_for_modify', {
      ...
      body: JSON.stringify(testModificationData)
    }).then(response => {
    ...
    .then(({data, status_code}) => {
      ...
      if (data.status === 'error') {
        ...
        alert(`Test modified successfully with ID: ${testId}`);
    ...
    .catch(error => {
      ...
      console.error('Error occurred in test modifcation process: ', error.message || error);
    });
  }
